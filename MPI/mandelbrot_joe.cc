...
main (int argc, char* argv[])
{
  ...
  double x, y;
	...
	MPI_Status status;
	MPI_Datatype small_block, big_block;
...
/////////////MPI/////////////
	...
	int **m_pts = (int **) malloc(N * sizeof(int *));
	for (int i=0; i<N; i++)
	{
	...
	MPI_Type_contiguous(N*width, MPI_INT, &small_block);
	MPI_Type_commit(&small_block);
	
	MPI_Type_contiguous(width*height, MPI_INT, &big_block);
	MPI_Type_commit(&big_block);
	
	//MPI_Barrier(MPI_COMM_WORLD);
	
	MPI_Gather(m_pts, N*width, small_block, M_pts, height*width, big_block, 0, MPI_COMM_WORLD);
